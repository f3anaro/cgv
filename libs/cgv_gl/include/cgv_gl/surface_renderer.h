...
namespace cgv { // @<
	namespace render { // @<
		...
		public:
			...
			template <typename T>
			void set_texcoord_array(const context& ctx, const T* texcoords, size_t nr_elements, unsigned stride_in_bytes = 0) { has_texcoords = true;  set_attribute_array(ctx, ref_prog().get_attribute_location(ctx, "texcoord"), texcoords, nr_elements, stride_in_bytes); }
