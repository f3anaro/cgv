...
void printtime()
{
  struct timeb timebuffer; // in windows you might have to use _timeb and _ftime
  char *timeline;
...
PlyFile* open_ply_for_read(const char* file_name)
{
	PlyFile* ply;
	FILE* fp;
...
PlyFile* open_ply_for_write(const char* file_name, int elem_num, char** elem_names, int file_type)
{
	PlyFile* ply;
	FILE* fp;
...
#define BIG_STRING 4096
  ...
  if (result) {
	  l = strlen(result)-1;
	  while ( (l >= 0) && ( (result[l] == 13) || (result[l] == 10) ) ) {
		  result[l] = 0;
		  --l;
	  }
  }
...
void get_binary_item_BE( // on windows
  FILE *fp,
  int type,
  int *int_val,
  unsigned int *uint_val,
...
void get_binary_item_LE( // on windows
  FILE *fp,
  int type,
  int *int_val,
  unsigned int *uint_val,
