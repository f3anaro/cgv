...
// Instead of depending on component <cgv_type/standard_types.h> here we use the
// standard integer definitions here. This prevents a circular dependency
// between component cgv_utils and cgv_types.
#include <cstdint>
...
{
	int16_t length;
	if (fread(&length, sizeof(int16_t), 1, fp) != 1)
		return false;
	s.resize(length);
	if (length == 0)
		return true;
...
{
	int16_t length = (int16_t)s.size();
	if (fwrite(&length, sizeof(int16_t), 1, fp) != 1)
		return false;
	if (length == 0)
		return true;
