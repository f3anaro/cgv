...
void connect(signal<@["T1"; ","; "T".i]>& s, void (*fp)(@["T1"; ", "; "T".i])) { s.connect(function_functor<@(i)@if(i>0)@{,@["T1"; ", "; "T".i]@}>(fp)); }
@}@else@{@//
extern void CGV_API connect(signal<@["T1"; ","; "T".i]>& s, void (*fp)(@["T1"; ", "; "T".i]));@}

// disconnect signal from a void function
@if(i > 0)@{@//
template <@["typename T1"; ", "; "typename T".i]>
void disconnect(signal<@["T1"; ","; "T".i]>& s, void (*fp)(@["T1"; ", "; "T".i])) { s.disconnect(function_functor<@(i)@if(i>0)@{,@["T1"; ", "; "T".i]@}>(fp)); }
@}@else@{@//
extern void CGV_API disconnect(signal<@["T1"; ","; "T".i]>& s, void (*fp)(@["T1"; ", "; "T".i]));@}
...
void connect(signal<@["T1"; ", "; "T".i]>& s, X* ip, void (X::*mp)(@["T1"; ", "; "T".i])) { s.connect(method_functor<@(i),X,X@if(i>0)@{,@["T1"; ", "; "T".i]@}>(ip,mp)); }
...
template <class Y, class X@if(i>0)@{, @["typename T1"; ", "; "typename T".i]@}>
void connect(signal<@["T1"; ", "; "T".i]>& s, X* ip, void (X::*mp)(@["T1"; ", "; "T".i])) { s.connect(method_functor<@(i),X,Y@if(i>0)@{,@["T1"; ", "; "T".i]@}>(ip,mp)); }

/// disconnect signal from a void method of an object
template <class X@if(i>0)@{, @["typename T1"; ", "; "typename T".i]@}>
void disconnect(signal<@["T1"; ", "; "T".i]>& s, X* ip, void (X::*mp)(@["T1"; ", "; "T".i])) { s.disconnect(method_functor<@(i),X,X@if(i>0)@{,@["T1"; ", "; "T".i]@}>(ip,mp)); }
...
template <class Y, class X@if(i>0)@{, @["typename T1"; ", "; "typename T".i]@}>
void disconnect(signal<@["T1"; ", "; "T".i]>& s, X* ip, void (X::*mp)(@["T1"; ", "; "T".i])) { s.disconnect(method_functor<@(i),X,Y@if(i>0)@{,@["T1"; ", "; "T".i]@}>(ip,mp)); }
...
template <class X@if(i>0)@{, @["typename T1"; ", "; "typename T".i]@}>
void connect(signal<@["T1"; ", "; "T".i]>& s, X* ip, void (X::*mp)(@["T1"; ", "; "T".i]) const) { s.connect(const_method_functor<@(i),X,X@if(i>0)@{,@["T1"; ", "; "T".i]@}>(ip,mp)); }

/// disconnect signal from a void const method of an object
template <class X@if(i>0)@{, @["typename T1"; ", "; "typename T".i]@}>
void disconnect(signal<@["T1"; ", "; "T".i]>& s, X* ip, void (X::*mp)(@["T1"; ", "; "T".i]) const) { s.disconnect(const_method_functor<@(i),X,X@if(i>0)@{,@["T1"; ", "; "T".i]@}>(ip,mp)); }
...
/*! This version of connect is necessary for class types X derived via multiple inheritance from cgv::signal::tacker 
	...
	if the tacker of Z is to be used. In most cases it does not matter which tacker is used. */
template <class Y, class X@if(i>0)@{, @["typename T1"; ", "; "typename T".i]@}>
void connect(signal<@["T1"; ", "; "T".i]>& s, X* ip, void (X::*mp)(@["T1"; ", "; "T".i]) const) { s.connect(const_method_functor<@(i),X,Y@if(i>0)@{,@["T1"; ", "; "T".i]@}>(ip,mp)); }
...
/*! This version of disconnect is necessary for class types X derived via multiple inheritance from cgv::signal::tacker 
	...
	as in the corresponding connect call!!! */
template <class Y, class X@if(i>0)@{, @["typename T1"; ", "; "typename T".i]@}>
void disconnect(signal<@["T1"; ", "; "T".i]>& s, X* ip, void (X::*mp)(@["T1"; ", "; "T".i]) const) { s.disconnect(const_method_functor<@(i),X,Y@if(i>0)@{,@["T1"; ", "; "T".i]@}>(ip,mp)); }
...
template <class X@if(i>0)@{, @["typename T1"; ", "; "typename T".i]@}>
void connect(signal<@["T1"; ", "; "T".i]>& s, X& ip) { s.connect(object_functor<@(i),X&@if(i>0)@{,@["T1"; ", "; "T".i]@}>(ip)); }
// disconnect signal from the ()-operator of a referenced object
template <class X@if(i>0)@{, @["typename T1"; ", "; "typename T".i]@}>
void disconnect(signal<@["T1"; ", "; "T".i]>& s, X& ip) { s.disconnect(object_functor<@(i),X&@if(i>0)@{,@["T1"; ", "; "T".i]@}>(ip)); }
// connect signal to the const ()-operator of a referenced const object
template <class X@if(i>0)@{, @["typename T1"; ", "; "typename T".i]@}>
void connect(signal<@["T1"; ", "; "T".i]>& s, const X& ip) { s.connect(object_functor<@(i),const X&@if(i>0)@{,@["T1"; ", "; "T".i]@}>(ip)); }
// disconnect signal from the const ()-operator of a referenced const object
template <class X@if(i>0)@{, @["typename T1"; ", "; "typename T".i]@}>
void disconnect(signal<@["T1"; ", "; "T".i]>& s, const X& ip) { s.disconnect(object_functor<@(i),const X&@if(i>0)@{,@["T1"; ", "; "T".i]@}>(ip)); }
