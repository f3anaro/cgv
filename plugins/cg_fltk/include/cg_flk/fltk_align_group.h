...
/** the align group provides a simple alignment mechanism for arranging
...
public:
	...
	fltk_align_group(int x, int y, int w, int h, const std::string& _name);
	/// delete fltk group realization
	~fltk_align_group();
	/// only uses the implementation of fltk_base
	std::string get_property_declarations();
	/// return the type name
	std::string get_type_name() const { return "fltk_align_group"; }
...
/// ref counted pointer to fltk_align_group
typedef cgv::data::ref_ptr<fltk_align_group> fltk_align_group_ptr;

#ifdef WIN32
CGV_TEMPLATE template class CGV_API cgv::data::ref_ptr<fltk_align_group>;
#endif
